...
**DECISION: 6 frameworks (not 9). Single-purpose, minimal tight coupling. Validated by Framework Decomposition Standard (FWK-0-DRAFT Section 3.0).**

The nine primitives are architectural concerns. Frameworks are governance units. Not 1:1. Three binding tests determine boundaries: (1) Splitting — independently authorable, (2) Merging — operational modes = one framework, (3) Ownership — exclusive data ownership.
...
├── Primitives: Install/uninstall gates, framework composition registry, version management, CLI tools
├── Responsibility: Validate packages, manage framework installation, maintain composition registry, provide operator CLI
├── Exposes: install-interface, query-composition-interface, CLI commands
...
│   └── SPEC-005-cold-storage-validation: Offline validation (connects directly to immudb, no kernel needed)
└── Contains: Gate runner, package validator, composition registry builder, CLI entry points, cold-storage validator
    NOTE: FMWK-007-kernel-cli merged here per decomposition standard (merging test: CLI tools
    are operational modes of the packaging capability, not a separate capability).
```
...
**WHY 6 NOT 9 (and not 7):**
- Three decomposition tests applied to every candidate (see FWK-0-DRAFT Section 3.0)
- CLI merged into Package Lifecycle: splitting test fails (CLI can't be authored independently from gates)
- Signal Accumulator is fold logic in Write Path: merging test (operational mode of folding)
- Work Order owned by Orchestration: ownership test (HO2 owns the lifecycle)
- Prompt Contract owned by Execution: ownership test (HO1 enforces them)
- Clear dependencies: acyclic (no circular dependencies)
- Operator-visible: "show frameworks" returns 6 items for KERNEL

**IMPLEMENTATION CHECKLIST:**
- [ ] KERNEL package contains exactly these 6 frameworks
- [ ] Each framework.json declares clear responsibility and interfaces
- [ ] Dependency graph: 006 ← {001, 002, 003, 004, 005} with no cycles
...
```
EXAMPLE: KERNEL package contains FMWK-001 through FMWK-006 with dependencies:
...
5. FMWK-006 (depends on 001, FWK-0)
...
KERNEL binary (monolithic, v1):
  ...
  - All compiled-in code is in KERNEL's responsibility boundary (the 6 frameworks)
...
- [ ] All 6 KERNEL frameworks listed with dependencies (no cycles)
