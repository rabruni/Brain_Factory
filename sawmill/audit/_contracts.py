...
SHELL_PROMPT_STEP_RE = re.compile(r"validate_prompt_step_success\s+([a-z0-9_]+)")
PYTHON_PROMPT_STEP_RE = re.compile(r'validate_prompt_step_success\(\s*"([a-z0-9_]+)"')
...
PYTHON_ARTIFACT_PATH_RE = re.compile(r'artifact_path\(\s*"([a-z][a-z0-9_]*)"\s*\)')
...
def parse_runtime_prompt_expectations(path: Path) -> tuple[dict[str, set[str]], set[str]]:
    ...
    if "validate_prompt_step_success(" in text:
        lines = text.splitlines()
        prompt_artifacts: dict[str, set[str]] = defaultdict(set)
        i = 0
        while i < len(lines):
            line = lines[i]
            prompt_match = PYTHON_PROMPT_STEP_RE.search(line)
            if not prompt_match:
                i += 1
                continue
            ...
            while i < len(lines) and paren_depth > 0:
                ...
                i += 1
            for artifact_id in PYTHON_ARTIFACT_PATH_RE.findall("\n".join(block)):
                prompt_artifacts[prompt_key].add(artifact_id)
        return prompt_artifacts, set()
    lines = text.splitlines()
    prompt_artifacts: dict[str, set[str]] = defaultdict(set)
    ...
    while i < len(lines):
        line = lines[i]
        prompt_match = SHELL_PROMPT_STEP_RE.search(line)
        if not prompt_match:
            i += 1
            continue
        prompt_key = prompt_match.group(1)
...
def parse_runtime_evidence_artifacts(path: Path) -> dict[str, set[str]]:
    try:
        ...
        raise ValueError(f"Missing run.sh: {path}") from exc
    if "validate_builder_evidence(" in text or "validate_reviewer_evidence(" in text or "validate_evaluator_evidence(" in text:
        evidence: dict[str, set[str]] = defaultdict(set)
        if "validate_builder_evidence(" in text:
            evidence["builder"].add("builder_evidence")
        if "validate_reviewer_evidence(" in text:
            evidence["reviewer"].add("reviewer_evidence")
        if "validate_evaluator_evidence(" in text:
            evidence["evaluator"].add("evaluator_evidence")
        return evidence
    evidence: dict[str, set[str]] = defaultdict(set)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        runtime_text_path = Path(args.run_sh)
        python_orchestrator_runtime = False
        try:
            run_sh_text = runtime_text_path.read_text(encoding="utf-8")
        except FileNotFoundError:
            run_sh_text = ""
        if "python3 -m sawmill.orchestrate" in run_sh_text:
            runtime_text_path = runtime_text_path.with_name("orchestrate.py")
            python_orchestrator_runtime = True
        runtime_prompt_expectations, background_prompts = parse_runtime_prompt_expectations(runtime_text_path)
        runtime_evidence_artifacts = parse_runtime_evidence_artifacts(runtime_text_path)
    ...
    if not isinstance(artifact_registry, dict) or not isinstance(prompt_registry, dict):
        ...
        return 1
    if python_orchestrator_runtime:
        runtime_prompt_expectations = {
            prompt_key: set(metadata["expected_artifacts"]) for prompt_key, metadata in prompt_registry.items()
        }
        background_prompts = set(prompt_registry)
    ...
    for artifact_id, owners in owners_per_artifact.items():
        if len(owners) != 1:
            record_failure(failures, "/".join(sorted(owners)), artifact_id, "artifact_registry", "exactly-one-owner-role")
    ...
    for role in COVERED_ROLES:
        ...
        for artifact_id in sorted(prompt_expected_by_role.get(role, set())):
            declared_sources = source_list(*sorted(prompt_sources_by_role_artifact[(role, artifact_id)]), f"runtime(PROMPT_REGISTRY:{artifact_id})")
            if artifact_id not in artifact_registry:
                record_failure(failures, role, artifact_id, declared_sources, "artifact_registry(ARTIFACT_REGISTRY.yaml)")
                continue
            if artifact_owner_by_id[artifact_id] != role:
                record_failure(
                    ...
                    role,
                    artifact_id,
                    source_list(*sorted(prompt_sources_by_role_artifact[(role, artifact_id)]), f"artifact_registry(owner={artifact_owner_by_id[artifact_id]})"),
                    f"artifact_registry(owner_role for {role})",
                )
            if artifact_id not in role_declared:
                record_failure(failures, role, artifact_id, declared_sources, f"role({ROLE_FILE_BY_ROLE[role]})")
            if artifact_id not in runtime_prompt_union:
                record_failure(failures, role, artifact_id, source_list(*sorted(prompt_sources_by_role_artifact[(role, artifact_id)])), "runtime(run.sh validate_prompt_step_success)")
            if not check_guard_covers_artifact(role, artifact_globs[artifact_id], role_guard_patterns):
                record_failure(
                    failures,
                    role,
                    artifact_id,
                    source_list(*sorted(prompt_sources_by_role_artifact[(role, artifact_id)]), f"role({ROLE_FILE_BY_ROLE[role]})", "runtime(PROMPT_REGISTRY/ARTIFACT_REGISTRY)"),
                    f"guard({args.guard})",
                )
        for artifact_id in sorted(role_declared):
            declared_by = f"role({ROLE_FILE_BY_ROLE[role]})"
            if artifact_id not in artifact_registry:
                record_failure(failures, role, artifact_id, declared_by, "artifact_registry(ARTIFACT_REGISTRY.yaml)")
                continue
            if artifact_owner_by_id[artifact_id] != role:
                record_failure(failures, role, artifact_id, source_list(declared_by, f"artifact_registry(owner={artifact_owner_by_id[artifact_id]})"), f"artifact_registry(owner_role for {role})")
            if artifact_id not in prompt_expected_by_role.get(role, set()):
                record_failure(failures, role, artifact_id, declared_by, "prompt_registry(PROMPT_REGISTRY.yaml)")
            if not check_guard_covers_artifact(role, artifact_globs[artifact_id], role_guard_patterns):
                record_failure(failures, role, artifact_id, declared_by, f"guard({args.guard})")
        for prompt_key, metadata in prompt_registry.items():
            if metadata["role"] != role:
                continue
            declared_expected = set(metadata["expected_artifacts"])
            runtime_expected = set(declared_expected) if prompt_key in background_prompts else runtime_prompt_expectations.get(prompt_key, set())
            for artifact_id in sorted(declared_expected - runtime_expected):
                record_failure(failures, role, artifact_id, f"prompt({prompt_key})", "runtime(run.sh validate_prompt_step_success)")
            for artifact_id in sorted(runtime_expected - declared_expected):
                record_failure(failures, role, artifact_id, "runtime(run.sh validate_prompt_step_success)", f"prompt({prompt_key})")
        for artifact_id in sorted(runtime_evidence_artifacts.get(role, set())):
            if artifact_id not in artifact_registry:
                record_failure(failures, role, artifact_id, "runtime(validate_evidence_artifact)", "artifact_registry(ARTIFACT_REGISTRY.yaml)")
                continue
            if artifact_owner_by_id[artifact_id] != role:
                record_failure(failures, role, artifact_id, source_list("runtime(validate_evidence_artifact)", f"artifact_registry(owner={artifact_owner_by_id[artifact_id]})"), f"artifact_registry(owner_role for {role})")
            if artifact_id not in prompt_expected_by_role.get(role, set()):
                record_failure(failures, role, artifact_id, "runtime(validate_evidence_artifact)", "prompt_registry(PROMPT_REGISTRY.yaml)")
            if not check_guard_covers_artifact(role, artifact_globs[artifact_id], role_guard_patterns):
                record_failure(failures, role, artifact_id, source_list("runtime(validate_evidence_artifact)", f"artifact_registry({artifact_id})"), f"guard({args.guard})")
    for role, patterns in guard_patterns.items():
        ...
        for pattern in sorted(patterns):
            ...
            if not matching_artifacts:
                record_failure(failures, role, pattern, f"guard({args.guard})", "artifact_registry(owner-matched artifact or explicit internal guard rule)")
    if failures:
