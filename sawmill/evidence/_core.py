"""Evidence hashing, validation, and report parsing."""
...
REVIEW_VERDICT_RE = re.compile(r"^[Rr]eview [Vv]erdict: (PASS|RETRY|ESCALATE)\s*$")
EVALUATION_VERDICT_RE = re.compile(r"^[Ff]inal [Vv]erdict: (PASS|FAIL)\s*$")
VERSION_TOKEN_RE = re.compile(r"^[0-9A-Za-z._-]+$")
...
def validate_reviewer(data: dict[str, Any], path: Path, args: argparse.Namespace) -> None:
    ...
    if data["verdict"] != "PASS" and not findings:
...
def parse_review_verdict(report_path: Path) -> str:
    match = REVIEW_VERDICT_RE.match(_last_nonempty_line(report_path))
    return match.group(1) if match else "UNKNOWN"


def parse_evaluation_verdict(report_path: Path) -> str:
    match = EVALUATION_VERDICT_RE.match(_last_nonempty_line(report_path))
    return match.group(1) if match else "UNKNOWN"


def extract_version_evidence(path: Path, label: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"Unable to parse version evidence from {path}") from exc
    for line in lines:
        line = line.rstrip("\r")
        if not line.startswith(label_prefix):
            continue
        label_count += 1
        value = line[len(label_prefix):].strip()
        if VERSION_TOKEN_RE.fullmatch(value):
            valid_count += 1
            valid_value = value
    if label_count == 0:
        raise ValueError(f"Missing required version evidence line in {path}: {label}: <version>")
    if label_count > 1:
        raise ValueError(f"Multiple '{label}:' lines found in {path}; expected exactly one")
    if valid_count != 1:
        raise ValueError(f"Malformed '{label}:' line in {path}; expected <version> token")
    return valid_value


def main_validate_evidence_artifacts(argv: list[str] | None = None) -> int:
